...
commander
  ...
  .option(
    "-t, --ci_project_path <ci-project-path>",
    "CodeBuild project relative path"
  )
  .option(
    "-j, --site_config <site-config>",
    "A JSON object contianing site configuration, overrides values defined in site config file"
  )
  .option(
    "-f, --site_config_file <site-config-path>",
...
const processConfiguration = async () => {
  ...
  projectParameters.site_config["ProjectS3Bucket"] = projectParameters
    ...
    : projectParameters.site_config["Domain"];
  projectParameters.ci_project_path = commander.ci_project_path
    ? commander.ci_project_path
    : paths.ci_project;
};
